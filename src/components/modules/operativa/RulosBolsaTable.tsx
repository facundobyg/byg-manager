...
function EliminarButton({ id }: { id: string }) {
  ...
  if (state?.ok) {
    return <span className="text-[9px] font-black text-byg-muted uppercase">Eliminado</span>;
  }
  ...
  if (!confirmed) {
    return (
      <button
        ...
        disabled={pending}
        className="p-2 text-byg-muted hover:text-red-500 transition-colors"
        title="Eliminar"
      >
        <Trash2 size={14} />
      </button>
    ...
    <form action={action} className="flex flex-col gap-1 items-center">
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          className="text-[9px] font-black px-2 py-0.5 rounded-full bg-byg-surface-2 text-byg-muted hover:bg-byg-border transition-colors"
        >
          Cancelar
        </button>
      </div>
        ...
        {cuentas.map((c) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${
              filtroCuenta === c ? "bg-slate-900 text-white shadow-md" : "bg-byg-bg text-byg-muted hover:bg-byg-border"
            }`}
          >
            {c}
          </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-byg-bg border-b border-byg-border">
                ...
                <th className="px-6 py-5 text-[10px] font-black uppercase text-byg-muted tracking-widest">Notas</th>
            ...
            <tbody className="divide-y divide-byg-border">
              {groups.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-20 text-center text-byg-muted font-medium italic">No se registraron rulos para esta selección.</td>
                </tr>
              ) : (
                    ...
                    <Fragment key={day}>
                      <tr className="bg-byg-bg">
                        <td colSpan={6} className="px-6 py-3">
                          <div className="flex items-center gap-2">
                            <CalendarDays size={14} className="text-byg-muted" />
                      ...
                      {dayRows.map((row) => (
                        <tr key={row.id} className="hover:bg-byg-surface-2 transition-colors group">
                          ...
                          <td className="px-6 py-4 text-[12px] text-byg-muted italic max-w-xs truncate">{row.notas}</td>
