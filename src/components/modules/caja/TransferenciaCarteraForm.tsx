...
import { transferirCajaACartera } from "@/app/(dashboard)/caja/actions";
...
export function TransferenciaCarteraForm({ cajas, carteras }: Props) {
  const formRef = useRef<HTMLFormElement>(null);
  const [state, action, pending] = useActionState(transferirCajaACartera, null);
    ...
    <div className="mb-8 w-full max-w-4xl">
      ...
      >
        <div className="flex items-center gap-3 border-b border-byg-border pb-4 mb-2">
          ...
          <div>
            <h3 className="text-sm font-black text-byg-text uppercase tracking-widest">
              Transferencia Caja → Cartera
            </h3>
            <p className="text-[10px] text-byg-muted font-bold uppercase mt-0.5">
              Fondea el saldo disponible de una cartera sin generar movimiento diario
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-byg-muted uppercase ml-1">Cartera Destino</label>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-byg-muted uppercase ml-1">Saldo destino</label>
            <select name="destinoSaldo" required className={sCls}>
              {moneda === "USD" ? (
                <>
        ...
        {state?.success && (
          <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-3">
            ...
            <p className="text-emerald-400 text-xs font-bold uppercase tracking-tight">
