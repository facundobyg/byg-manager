...
import { updateTCMep } from "@/app/(dashboard)/configuracion/actions";

type Props = { valorActual: string | null };

const init: { error?: string; ok?: boolean } = {};

export function TcMepForm({ valorActual }: Props) {
  const [state, action, pending] = useActionState(updateTCMep, init);
  ...
  return (
    <form action={action} className="flex flex-col gap-4">
      ...
      {state.ok    && <p className="text-xs text-green-600">TC MEP actualizado correctamente.</p>}
    </form>
  );
}
